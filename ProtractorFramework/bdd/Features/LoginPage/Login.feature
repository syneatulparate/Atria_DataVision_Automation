Feature: Authentication & Authorization
    ...
    Scenario Outline: User Locked out for 3 times invalid security answer
        ...
        Then  User expect that "<ErrorMsg>" is displayed
        Examples:
            | username     | password | SecurityAnswer | email                      | ErrorMsg                                                                |
            | CLASSICDEV27 | drowssap | 123456         | classicdev13@synechron.com | We're sorry but you have been locked out due to too many login failures |
    ...
    Scenario Outline: Error message for Invalid Security answer
        ...
        When User enters valid "<username>" and "<password>" and click login button
        Then  User expect that the SecurityAnswer field is displayed
        When  User enter invalid SecurityAnswer "<SecurityAnswer>" and click on Login button
        Then  User expect that "<ErrorMsg>" is displayed
        Examples:
            | username     | password | email                      | SecurityAnswer | ErrorMsg   |
