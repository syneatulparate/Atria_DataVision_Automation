...
When(
  /^User enters "([^"]*)?" into the Username field and click on the continue button$/,
  async username => {
...
Then(/^User expect that "([^"]*)?" is displayed$/, async ErrorMsg => {
  await staticWait(3000);
  expect(await forgotPasswordPage.verifyErrorMessage(ErrorMsg)).to.equal(
    true,
    "error message is not displayed"
  );
});
Then(/^User expect that "([^"]*)?" is present$/, async ErrorMsg => {
  await forgotPasswordPage.verifyErrorMessage(ErrorMsg);
});

Then(/^User enters "([^"]*)?"$/, async emailAddress => {
  await forgotPasswordPage.enterInvalidEmail(emailAddress);
});

Then(/^User expect that email field is displayed$/, async () => {
  await staticWait(3000);
  expect(await forgotPasswordPage.verifyEmailField()).to.equal(
    true,
    "email file is not present"
  );
});

When(
  /^User set "([^"]*)?" to the answer field and "([^"]*)?" into email field and clicks on continue button$/,
  async (securityAnswer, emailAddress) => {
...
When(
  /^User enters "([^"]*)?" into the OTP field and clicks on continue button$/,
  async OTP => {
...
When(
  /^User enters a invalid email "([^"]*)?" in email textfield and clicks on continue button$/,
  async email => {
...
When(
  /^User click on the forgot password link and navigates to Forgot password Page$/,
  async () => {
    return await loginPage.txtUsername.isDisplayed().then(
      ...
      async isDisplayed => {
        return await loginPage.logoutBtn.isDisplayed().then(
          async isDisplayed => {
