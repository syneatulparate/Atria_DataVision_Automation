...
Given(/^User is on Login page$/, openWebsite);

When(
  /^User enters valid "([^"]*)?" and "([^"]*)?" and click login button$/,
...
When(
  /^User click on the forgot password link and verifies the title "([^"]*)?" is displayed$/,
  async headerMessage => {
...
Then(
  /^User expects that title contains the text "([^"]*)?"$/,
  async headerMessage => {
...
When(
  /^User enter securityAnswer to the securityAnswer input field$/,
  async () => {
...
When(
  /^User click on the logout button and verifies the title "([^"]*)?" is Present$/,
  async headerMessage => {
    await loginPage.clickOnLogOut();
    await loginPage.verifyLoginPageTitle(headerMessage);
  }
...
When(
  /^User enter securityAnswer to the securityAnswer input field and click on Loginandrememberme button$/,
  async () => {
    await loginPage.enterSecurityAnswer();
...
When(
  /^User enter invalid otp "([^"]*)?" and click on Login button$/,
  async ANS => {
...
When(
  /^User enter invalid SecurityAnswer "([^"]*)?" and click on Login button$/,
  async ANS => {
