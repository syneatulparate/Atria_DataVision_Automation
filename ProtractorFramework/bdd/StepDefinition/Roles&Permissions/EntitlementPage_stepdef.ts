...
Then(/^User expects that "([^"]*)?" link is not displayed on the dashboard page$/, async () => {
    await waitForObject(loginPage.logoutBtn)
...
Then(/^Admin User is navigated to the Role Detail Page of the corresponding "([^"]*)?"$/, async () => {
    await waitForObject(loginPage.logoutBtn)
...
Then(/^Admin User is navigated to the Permission Detail Page of the corresponding "([^"]*)?"$/, async () => {
    await waitForObject(loginPage.logoutBtn)
    await loginPage.verifyLoginPageTitle("Roles & Permissions – dataVISION")
})
