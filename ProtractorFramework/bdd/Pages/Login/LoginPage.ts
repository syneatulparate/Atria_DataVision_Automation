...
export class LoginPage {
  ...
  constructor() {
    ...
    this.headerMsg = element(
      by.css("div.ng-star-inserted:nth-child(1) > h1:nth-child(1)")
    );
    this.txtOTP = element(
      by.xpath("//input[@id='code'] | //input[@id='otp-code']")
    );
    this.txtEmail = element(by.css("input#security-email"));
    this.errMsg = element(by.xpath("//div[@class='toast-message']"));
    this.errMsg = element(by.xpath("//dt")).element(
      by.xpath("//a[@routerlink='users']")
    );
  }
  /**
   * To Enter UserName
