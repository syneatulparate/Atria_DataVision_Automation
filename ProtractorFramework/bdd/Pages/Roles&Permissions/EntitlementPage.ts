...
export class EntitlementPage {
  ...
  constructor() {
    this.addUserButton = element(
      by.xpath("//button[contains(text(),  '+ Add Users')]")
    );
    ...
    this.searchButton = element(
      by.css("td.td-align-right button.btn.btn-small")
    );
    this.deleteRoleButton = element(
      by.xpath("//a[contains(text(),'Delete Role')]")
    );
    this.addSelectedUserButton = element(
      by.css("button.btn.btn-small[type='submit']")
    );
    this.messageBox = element(by.xpath("//div[@class='toast-message']"));
    this.userProfile = element(
      by.xpath("//*[contains(text(),'User Profile')]")
    );
    this.classicUserOwnerships = element(
      by.xpath("//th[contains(text(),  'Classic User Ownerships')]")
    );
    this.classicContact = element(
      by.xpath("//th[contains(text(),  'Classic Contact Allow Read Scope')]")
    );
    this.usersLink = element(by.xpath("//a[@routerlink='users']"));
    this.brokersDealersLink = element(by.css("a[routerlink='broker-dealers']"));
    this.classicContactViewLink = element(
      by.xpath("//a[@routerlink='system-usage/classic-contact-views']")
    );
    ...
    this.sendMyDocsSavesLink = element(
      by.css("a[routerlink='user-content/scans-saves']")
    );
    this.sendMyDocsScanners = element(
      by.css("a[routerlink='user-content/scanners']")
    );
    this.sendMyScansLink = element(
      by.css("a[routerlink='user-content/scans']")
    );
    this.userSessionsLink = element(
      by.css("a[routerlink='system-usage/user-sessions']")
    );
    this.manageUserLink = element(
      by.css("a[routerlink='mpv-administration/manage-users']")
    );
    this.activeLockouts = element(
      by.css("a[routerlink='mpv-administration/active-lockouts']")
    );
    this.workGroupsLink = element(by.css("a[routerlink='workgroups']"));
    this.adminButton = element(by.css("a[routerlink='admin']"));
    this.userList = element.all(
      by.xpath(
        "//table//th[contains(text(),'Assign Users This Role')]//ancestor::table[1]//tr/td[1]/span[1]"
      )
    );
    this.searchTextGlobal = element(
      by.css("input[placeholder ='Search dataVISION']")
    );
    this.searchBtnGlobal = element(by.id("searchDV"));
    this.NewRoleButton = element(
      by.xpath("//button[contains(text(),'+ New Role')]")
    );
    this.txtRoleName = element(by.xpath("//input[@id='newRoleName']"));
    this.CreateRoleButton = element(
    ...
    this.warningMsg = element(
      by.xpath(
        "//p[contains(text(),  'Role name is limited to 50 characters')]"
      )
    );
    ...
    this.RolesandPermissionSection = element(
      by.xpath("//th[contains(text(),  'Roles & Permissions')]")
    );
    this.ContactHovertab = element(
      by.xpath("//div[contains(text(),'Contacts')]")
    );
    this.contactDirectory = element(
    ...
    this.comissionslink = element(
      by.xpath("//a[contains(text(),'Commissions')]")
    );
    this.ToolsTab = element(by.xpath("//div[contains(text(),'Tools')]"));
    this.orgchatlink = element(by.xpath("//a[contains(text(),'Org Chart')]"));
    this.cfsenggbloglink = element(
      by.xpath("//a[contains(text(),'CFS/SPF Engineering Blog')]")
    );
    this.setupclientscanner = element(
      by.xpath("//a[contains(text(),'+ Set Up Client Scanner')]")
    );
    this.ComisInterpreterslink = element(
      by.xpath("//a[contains(text(),'Commissions Interpreters')]")
    );
    this.ResourcesTab = element(by.xpath("//*[contains(text(),'Resources')]"));
    this.UserTab = element(
      by.xpath("//nav[@class='masthead-nav']/div[2]/div[2]")
    );
    this.UpdateProfileBtn = element(
      by.xpath("//a/span[text()='Update Profile']")
    );
    this.searchtextbox = element(
      by.css("input[placeholder ='Search dataVISION']")
    );
    this.Summeryele = element(by.css("h2.hr"));
    this.Usersettings = element(
      by.xpath("//a[@ng-reflect-router-link='settings/user/profile']")
    );
    this.rolesfiltertxt = element(by.css("input[placeholder='Filter Roles']"));
    this.PermissionsFiltertxt = element(
      by.css("input[placeholder='Filter Permissions']")
    );
    this.UsersFiltertxt = element(by.css("input[placeholder='Filter Users']"));
    this.roleList = element.all(
      by.xpath(
        "//table//th[contains(text(),'Role')]//ancestor::table[1]//span[1]"
      )
    );
    this.permissionList = element.all(
      by.xpath(
        "//table//th[contains(text(),'Permissions')]//ancestor::table[1]//tbody//tr/td[1]"
      )
    );
    this.UsersList = element.all(
      by.xpath(
        "//table//th[contains(text(),'Users')]//ancestor::table[1]//td[1]"
      )
    );
    this.settingslink = element(
      by.css("a[routerlink='settings/usersettings']")
    );
  ...
  verifyGlobalSearchTexbox = async () => {
    let flag: any;
    return await isElementPresent(this.searchTextGlobal).then(function(flag) {
      return flag;
    });
  };
  ...
  selectUser = async userName => {
    let checkBoxForUser = element(
      by.xpath("//*[(text()='" + userName + "')]//ancestor::tr//tbody//input")
    );
  ...
  verifyRoleNotPresent = async () => {
    let linkPresent: Promise<boolean>;
    linkPresent = this.verifyElement(
      element(
        by.xpath(
          "//*[@dv-tooltip='" +
            this.randonname +
  ...
  verifyRoleandPermissionColumn = async () => {
    ...
    linkPresent = this.verifyElement(
      element(by.xpath("//th[contains(text(),'Users Assigned This Role')]"))
    );
    linkPresent = this.verifyElement(
  ...
  deSelectCheckBox = async permission => {
    let permissionCheckbox = element(
      by.xpath(
        "//*[text()='" +
          permission +
          "']/ancestor::tr[@class='ng-star-inserted']//input | //*[@dv-tooltip='" +
  ...
  SelectMultipleCheckBox = async () => {
    element
      .all(by.css("span input.checkbox"))
      .isSelected()
  ...
  async VerifyPermission(permission): Promise<boolean> {
    let linkPresent: Promise<boolean>;
    let permissionelement1 = element(
      by.xpath(
        "//*[text()='" +
          permission +
          "'] | //*[@dv-tooltip='" +
  ...
  createRandomName = async length => {
    ...
    for (i = 0; i < length; i++) {
      this.randonname += letters.charAt(
  ...
  elemetbeforeEdit = async () => {
    let i;
    let elem = element(
      by.xpath("//div[@class='tbl']//tbody/tr/td[@colspan='2']")
    );
  ...
  verifyPermissionCancel = async () => {
    let i;
    let elem = element(by.css(".tbl-body.zebrastripe tr"))
      .count()
      .then(function(rowCount) {
        return rowCount;
      });
    let counteleafter = elem;
    let elem1 = element(by.css(".tbl-body.zebrastripe tr"))
      .count()
      .then(function(rowCount) {
        return rowCount;
      });
  ...
  verifyCommissionsInterpretersLink = async () => {
    let flag: any;
    return await isElementPresent(this.ComisInterpreterslink).then(function(
      flag
    ) {
      return flag;
    });
  ...
  verifyClassicContactViewsLink = async () => {
    let flag: any;
    return await isElementPresent(this.classicContactViewLink).then(function(
      flag
    ) {
      return flag;
    });
  ...
  verifyRolesAndPermissionsLink = async () => {
    let flag: any;
    return await isElementPresent(this.rolesAndPermissionLink).then(function(
      flag
    ) {
      return flag;
    });
  ...
  verifySendMyDocsSavesLink = async () => {
    let flag: any;
    return await isElementPresent(this.sendMyDocsSavesLink).then(function(
      flag
    ) {
      return flag;
    });
  ...
  verifyScreenTiltle = async title => {
    await isElementPresent(
      element(by.xpath("//h1[contains(text(),'" + title + "')]"))
    );
  };
  ...
  clickOnUserName = async linkname => {
    let ele = element(
      by.xpath(
        "//div[@class='tbl']//tbody/tr//a[contains(text(),'" + linkname + "')]"
      )
    );
  };
  ...
  afterEnterRole = async role => {
    this.rows1 = element.all(
      by.xpath(
        "//tbody[@class='tbl-body zebrastripe ng-star-inserted']//tr//span[contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz') , translate('" +
          role +
  ...
  verifyElementRoleAndpermision = async elem => {
    let flag: any;
    return await isElementPresent(
      element(by.xpath("//h1[contains(text(),'Roles & Permissions')]"))
    ).then(function(flag) {
      return flag;
    });
  };
